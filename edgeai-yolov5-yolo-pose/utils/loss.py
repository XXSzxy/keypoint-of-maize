...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        sigmas = torch.tensor([.25, .25, .25, .25, .25, .25, .25, .25, .25, .25,.25, .25, .25, .25, .25, .25, .25, .25, .25, .25][:12], device=device) / 3.0
    ...
    def build_targets(self, p, targets):
        ...
        if self.kpt_label:
            gain = torch.ones(31, device=targets.device).long()  # normalized to gridspace gain
        else:
        ...
        for i in range(self.nl):
            ...
            if self.kpt_label:
                gain[2:30] = torch.tensor(p[i].shape)[14*[3, 2]]  # xyxy gain
            else:
