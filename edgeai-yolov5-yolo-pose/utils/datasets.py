...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.flipLR_index = [1,0,3,2,5,4,7,6,9,8,11,10,13,12,15,14,17,16,19,18][:12]
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            if kpt_label:
                xy_kpts = np.ones((n * 12, 3))
                xy_kpts[:, :2] = targets[:,5:].reshape(n*12, 2)  #num_kpt is hardcoded to 17
                xy_kpts = xy_kpts @ M.T # transform
                xy_kpts = (xy_kpts[:, :2] / xy_kpts[:, 2:3] if perspective else xy_kpts[:, :2]).reshape(n, 24)  # perspective rescale or affine
                xy_kpts[targets[:,5:]==0] = 0
                x_kpts = xy_kpts[:, list(range(0,24,2))]
                y_kpts = xy_kpts[:, list(range(1,24,2))]
                ...
                xy_kpts[:, list(range(0, 24, 2))] = x_kpts
                xy_kpts[:, list(range(1, 24, 2))] = y_kpts
